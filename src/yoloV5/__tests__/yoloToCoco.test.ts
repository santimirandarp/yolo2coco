...
let cocoTrue: CocoDatasetFormat;
let jsonResult: CocoDatasetFormat;
...
beforeAll(async () => {
  ...
  jsonResult = (await yoloV5ToCoco(join(dataPath, yamlFile))).val;
});
...
test('Compare one same Image Item', () => {
  ...
  expect(bbox[0]).toBeCloseTo(bboxTrue[0], -1);
  expect(bbox[1]).toBeCloseTo(bboxTrue[1], -1);
  expect(bbox[2]).toBeCloseTo(bboxTrue[2], -1);
  expect(bbox[3]).toBeCloseTo(bboxTrue[3], -1);
});
